...
│                   (localhost:5001)    (localhost:11434)           │
│                                                                  │
...
### 3. Download sql.js files

These must be present in the VoxLog folder alongside `index.html`. Download them with Python:
...
VoxLog requires **three things running** before opening the browser. Open three separate PowerShell windows:
...
Edit `whisper_server.py` and change `MODEL_SIZE` at the top:
...
| Problem | Fix |
|---------|-----|
| "Cannot reach Whisper server" | Make sure `python whisper_server.py` is running in its own window |
| "Cannot reach Ollama" | Run `ollama serve` in a separate window |
| Browser shows old version | Hard refresh with `Ctrl+Shift+R` |
| `http://localhost:8080` not loading | Use `http://127.0.0.1:8080` instead |
| Transcription slow | Switch `MODEL_SIZE` to `tiny` in `whisper_server.py` |
...
- 🏛️ No OCIO/RMC approval required for external AI — this is fully on-premise
- 📋 Transcription data stays in your browser and local `.db` file only
- 🗑️ `.db` files should be treated as records per your retention schedule
...
When the **Local AI Setup** modal appears, click **Connect & Continue →**. If you get an error, let IT know — the background services may not be started.
...
4. Files appear in the dashboard immediately — you can add more files at any time
...
4. When complete, each row will show:
   - The full transcription text
   - Caller name (auto-extracted)
   - Phone number (auto-extracted)
   - Call date and time (auto-stamped)

> ⏱️ Transcription runs locally — speed depends on file length. A typical 1-minute voicemail takes about 10–20 seconds.
...
Use the Status column to track where each voicemail is in your workflow:
...
- **Search bar** — type anything to filter across all columns (name, phone, transcript text, notes, date, filename)
...
Click **💾 Backup** to save everything to a `voxlog.db` file on your computer. You can reload it next session with **📂 Load Backup**.
...
When records are ready to sync to the VoxLog SharePoint List:
...
> 📧 Do not change the email subject — Power Automate uses it as a trigger.

---

## Exporting to Excel

Click **⬇ Export CSV** to download all records as a `.csv` file that opens in Excel or any spreadsheet app.
...
- If a transcription looks wrong, just click the cell and correct it manually

---

## Need Help?

Contact your IT department or refer to the full documentation in this README.
